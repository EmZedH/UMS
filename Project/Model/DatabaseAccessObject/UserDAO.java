...
public class UserDAO extends Connect{
    ...
    public List<List<String>> selectUserInCollege(int collegeID) throws SQLException {
        return createArrayFromTable("SELECT USER_ID, U_NAME, U_AADHAR, U_DOB, U_GENDER, U_ADDRESS, U_PASSWORD FROM USER LEFT JOIN (SELECT STUDENT_ID AS USER_ID, COLLEGE_ID FROM STUDENT UNION SELECT PROF_ID AS USER_ID, COLLEGE_ID FROM PROFESSOR UNION SELECT CA_ID AS USER_ID, COLLEGE_ID FROM COLLEGE_ADMIN) ON USER.U_ID = USER_ID WHERE COLLEGE_ID = "+collegeID, new String[]{"USER_ID","U_NAME","U_AADHAR","U_DOB","U_GENDER","U_ADDRESS","U_PASSWORD"});
    }

    public List<List<String>> searchUserInCollege(String column, String searchString, int collegeID) throws SQLException {
