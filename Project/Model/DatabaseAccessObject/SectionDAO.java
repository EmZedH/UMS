...
public class SectionDAO extends Connect{
    ...
    public String[][] selectAllSection() throws SQLException {
        return createArrayFromTable("SELECT SEC_ID,SEC_NAME,DEPT_NAME,C_NAME FROM SECTION LEFT JOIN DEPARTMENT ON (DEPARTMENT.DEPT_ID = SECTION.DEPT_ID AND SECTION.COLLEGE_ID = DEPARTMENT.COLLEGE_ID) LEFT JOIN COLLEGE ON COLLEGE.C_ID = SECTION.COLLEGE_ID", new String[]{"SEC_ID","SEC_NAME","DEPT_NAME","C_NAME"});
    }

    public String[][] searchAllSection(String column, String searchString) throws SQLException {
    ...
    public String[][] selectSectionInCollege(int collegeID) throws SQLException {
        return createArrayFromTable("SELECT SEC_ID,SEC_NAME,DEPT_NAME FROM SECTION LEFT JOIN DEPARTMENT ON (DEPARTMENT.DEPT_ID = SECTION.DEPT_ID AND SECTION.COLLEGE_ID = DEPARTMENT.COLLEGE_ID) LEFT JOIN COLLEGE ON COLLEGE.C_ID = SECTION.COLLEGE_ID WHERE C_ID = "+collegeID, new String[]{"SEC_ID","SEC_NAME","DEPT_NAME"});
    }

    public String[][] searchSectionInCollege(String column, String searchString, int collegeID) throws SQLException {
        return createArrayFromTable("SELECT SEC_ID,SEC_NAME,DEPT_NAME FROM (SELECT SEC_ID,SEC_NAME,DEPT_NAME,SECTION.COLLEGE_ID,1 AS TYPE FROM SECTION LEFT JOIN DEPARTMENT ON (DEPARTMENT.DEPT_ID = SECTION.DEPT_ID AND SECTION.COLLEGE_ID = DEPARTMENT.COLLEGE_ID) LEFT JOIN COLLEGE ON COLLEGE.C_ID = SECTION.COLLEGE_ID WHERE SECTION.COLLEGE_ID = "+collegeID+" AND "+column+" LIKE '"+searchString+"%' UNION SELECT * FROM (SELECT SEC_ID,SEC_NAME,DEPT_NAME,SECTION.COLLEGE_ID,2 AS TYPE FROM SECTION LEFT JOIN DEPARTMENT ON (DEPARTMENT.DEPT_ID = SECTION.DEPT_ID AND SECTION.COLLEGE_ID = DEPARTMENT.COLLEGE_ID) LEFT JOIN COLLEGE ON COLLEGE.C_ID = SECTION.COLLEGE_ID WHERE SECTION.COLLEGE_ID = "+collegeID+" AND "+column+" LIKE '%"+searchString+"%' EXCEPT SELECT SEC_ID,SEC_NAME,DEPT_NAME,SECTION.COLLEGE_ID,2 AS TYPE FROM SECTION LEFT JOIN DEPARTMENT ON (DEPARTMENT.DEPT_ID = SECTION.DEPT_ID AND SECTION.COLLEGE_ID = DEPARTMENT.COLLEGE_ID) LEFT JOIN COLLEGE ON COLLEGE.C_ID = SECTION.COLLEGE_ID WHERE SECTION.COLLEGE_ID = "+collegeID+" AND "+column+" LIKE '"+searchString+"%')) ORDER BY TYPE", new String[]{"SEC_ID","SEC_NAME","DEPT_NAME"});
    }
